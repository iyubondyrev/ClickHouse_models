...
int main(int, char **argv)
{
    ...
    const char* token_types_filename = argv[1];
    const char* tokens_filename = argv[2];


    int fd_1 = open(token_types_filename, O_WRONLY | O_CREAT | O_APPEND, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
    if (fd_1 == -1) {
        perror("Error opening file");
        return 1;
    }

    int fd_2 = open(tokens_filename, O_WRONLY | O_CREAT | O_APPEND, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
    if (fd_2 == -1) {
        perror("Error opening file");
        return 1;
    }
    ...
    while (true)
    {
        ...
        write(fd_2, token.begin, token.size());
        write(fd_2, &sep, 1);
        ...
        const char * token_type_str = getTokenName(token.type);
        write(fd_1, token_type_str, strlen(token_type_str));
